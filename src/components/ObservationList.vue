...
<template>
  ...
  <slot name="default">
    ...
    <nav class="nav" v-if="props.observations.length">
      ...
      <a v-if="props.comments" href="#comments" class="nav-link" :class="{ current: sort == 'comments', }" @click.prevent="emitSort('comments')" :title="t('Comments')"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 16 16"><g fill="currentColor"><path d="M14.853,6.861c-.729,3.487-4.193,6.139-8.353,6.139-.102,0-.201-.016-.302-.019,1.035,.637,2.359,1.019,3.802,1.019,.51,0,1.003-.053,1.476-.143l2.724,2.043c.088,.066,.193,.1,.3,.1,.076,0,.153-.018,.224-.053,.169-.085,.276-.258,.276-.447v-3.515c.631-.712,1-1.566,1-2.485,0-.987-.429-1.897-1.147-2.639Z"></path><path d="M6.5,0C2.91,0,0,2.462,0,5.5c0,1.075,.37,2.074,1,2.922v3.078c0,.189,.107,.362,.276,.447,.07,.035,.147,.053,.224,.053,.106,0,.212-.034,.3-.1l1.915-1.436c.845,.34,1.787,.536,2.785,.536,3.59,0,6.5-2.462,6.5-5.5S10.09,0,6.5,0Z"></path></g></svg> <span class="nav-count">({{ noOfComments() }})</span></a>
    </nav>
