...
/* Comments */
async function addComment(listId) {
...
<template>
  ...
  <slot name="default">
    ...
    <nav class="nav" v-if="props.observations.length">
      <a href="#bydate" class="nav-link" :class="{ current: sort == 'bydate', }" @click.prevent="emitSort('bydate')"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 16 16"><g transform="translate(0, 0)"><path fill="currentColor" d="M15.2,7.7L15.2,7.7c-0.8-1.1-2.9-3.9-3.7-5l0,0C11.1,2.3,10.6,2,10,2C8.9,2,8,2.9,8,4c0-1.1-0.9-2-2-2 C5.4,2,4.9,2.3,4.5,2.7l0,0c-0.8,1-2.9,3.8-3.7,5l0,0C0.3,8.3,0,9.1,0,10c0,2.2,1.8,4,4,4s4-1.8,4-4c0,2.2,1.8,4,4,4s4-1.8,4-4 C16,9.1,15.7,8.3,15.2,7.7z M4,12c-1.1,0-2-0.9-2-2s0.9-2,2-2s2,0.9,2,2S5.1,12,4,12z M12,12c-1.1,0-2-0.9-2-2s0.9-2,2-2s2,0.9,2,2 S13.1,12,12,12z"></path></g></svg> 
        {{ t("Observations") }}
        <span class="nav-count">({{ observationsByUser.length }})</span>
      </a>
      <a href="#byname" class="nav-link" :class="{ current: sort == 'byname', }" @click.prevent="emitSort('byname')"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 16 16"><g transform="translate(0, 0)"><path fill="currentColor" d="M14.128,7.084c-.443-.946-.861-1.841-.878-3.483-.019-1.888-1.447-3.465-3.25-3.592-.568-.04-1.116,.054-1.633,.277-.233,.102-.5,.102-.734,0C7.116,.062,6.569-.03,6,.009c-1.803,.127-3.231,1.704-3.25,3.592-.017,1.643-.435,2.537-.878,3.483-.429,.916-.872,1.862-.872,3.416,0,3.032,2.467,5.5,5.5,5.5h3c3.033,0,5.5-2.468,5.5-5.501,0-1.553-.443-2.5-.872-3.415ZM5,3.5c.551,0,1,.448,1,1s-.449,1-1,1-1-.448-1-1,.449-1,1-1Zm6.972,9.211c-.047,.134-.149,.242-.28,.297l-1.197,.498c-.792,.329-1.644,.494-2.495,.494s-1.702-.165-2.495-.494l-1.197-.498c-.131-.055-.233-.163-.28-.297s-.034-.282,.034-.406l1.419-2.573,.558-2.578c.179-.927,1.005-1.607,1.961-1.607,.944,0,1.768,.67,1.959,1.594l.56,2.592,1.419,2.573c.068,.124,.081,.272,.034,.406Zm-.972-7.211c-.551,0-1-.448-1-1s.449-1,1-1,1,.448,1,1-.449,1-1,1Z"></path></g></svg> 
        {{ t("Species") }}
        <span class="nav-count">({{ Object.keys(speciesByUser).length }})</span>
      </a>
      <a v-if="props.comments" href="#comments" class="nav-link" :class="{ current: sort == 'comments', }" @click.prevent="emitSort('comments')"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 16 16"><g fill="currentColor"><path d="M14.853,6.861c-.729,3.487-4.193,6.139-8.353,6.139-.102,0-.201-.016-.302-.019,1.035,.637,2.359,1.019,3.802,1.019,.51,0,1.003-.053,1.476-.143l2.724,2.043c.088,.066,.193,.1,.3,.1,.076,0,.153-.018,.224-.053,.169-.085,.276-.258,.276-.447v-3.515c.631-.712,1-1.566,1-2.485,0-.987-.429-1.897-1.147-2.639Z"></path><path d="M6.5,0C2.91,0,0,2.462,0,5.5c0,1.075,.37,2.074,1,2.922v3.078c0,.189,.107,.362,.276,.447,.07,.035,.147,.053,.224,.053,.106,0,.212-.034,.3-.1l1.915-1.436c.845,.34,1.787,.536,2.785,.536,3.59,0,6.5-2.462,6.5-5.5S10.09,0,6.5,0Z"></path></g></svg> 
        {{ t("Comments") }}
    ...
    <section id="comments" v-if="currentList && props.sort == 'comments'">
      <form>
        <div>
          ...
          <button class="comment-btn" @click.prevent="addComment(currentList.listId)">{{ t("Submit") }}</button>
        </div>
      </form>
      <transition-group tag="ol" name="list" class="list">
